...
class Vector
{
   public:
	  ...
	  */
      ...
      /*!
	  * Function adding the scaled square (component-wise) of a vector to the current one
	  * @param[in] v
	  */
      Vector& sqsadd(const Vector& v, const double& c);  
...
// Scaled Squared (component-wise) of three vectors to an existing vector
inline
Vector& Vector::sqsadd(const Vector& v, const double& c)
{                     
   x += v.x*v.x*c;
   y += v.y*v.y*c;
   z += v.z*v.z*c;
