...
{
   public:
      Grid(char *file, char *loc, int NPART);
      
      unsigned int NPART;         /*!< number of mesh partitions requested */
   ...
   private:
	  ...
	  * Remove pseudo files
	  */
      void remove_pseudo_files();

};

template <int DIM>
inline
Grid<DIM>::Grid(char *file, char *loc, int NPART):
NPART(NPART)
{
